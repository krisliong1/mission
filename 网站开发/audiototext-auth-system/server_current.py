...
from subscription_routes import router as sub_router
from database import get_db
from middleware import check_user_limit, record_usage, check_ip_limit, increment_ip_usage

app = FastAPI()

# CORS — allow audiototext.oskris.com + localhost dev
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://audiototext.oskris.com",
        "http://localhost:3000",
        "http://localhost:8080",
        "http://127.0.0.1:3000",
    ],
...
_webhook = _APIRouter(prefix="/api/webhook", tags=["webhook"])
@_webhook.post("/stripe")
async def stripe_webhook(request: Request):
    return {"received": True}
app.include_router(_webhook)
...
async def transcribe_url(
    ...
    limit_check = check_user_limit(current_user["id"], plan, db)
    if not limit_check["allowed"]:
        db.close()
        raise HTTPException(status_code=429, detail=limit_check)
...
async def transcribe_audio_upload(
    ...
    plan2 = sub2["plan"] if sub2 else "free"
    lc2 = check_user_limit(current_user["id"], plan2, db2)
    if not lc2["allowed"]:
        db2.close()
        raise HTTPException(status_code=429, detail=lc2)
    if not check_rate_limit(ip):
        raise HTTPException(status_code=429, detail="请求过于频繁，请稍后再试")
    tmp_path = None
    try:
...
async def transcribe_video_upload(
    ...
    plan3 = sub3["plan"] if sub3 else "free"
    lc3 = check_user_limit(current_user["id"], plan3, db3)
    if not lc3["allowed"]:
        db3.close()
        raise HTTPException(status_code=429, detail=lc3)
    if not check_rate_limit(ip):
        raise HTTPException(status_code=429, detail="请求过于频繁，请稍后再试")
    tmp_path = None
    audio_path = None
